...
def create_model(input_shape, anchors, num_classes, load_pretrained=True, freeze_body=2,
    ...
    # print("bottleneck_model.output[0].shape :    ========",bottleneck_model.output[0].shape[1:].as_list())
    # print("bottleneck_model.output[1].shape :    ========", bottleneck_model.output[1].shape[1:].as_list())
    # print("bottleneck_model.output[2].shape :    ========", bottleneck_model.output[2].shape[1:].as_list())

    # create last layer model of last layers from yolo model
    in0 = Input(shape=bottleneck_model.output[0].shape[1:].as_list()) #in0 = Input(shape=[None, None, 1024])
    in1 = Input(shape=bottleneck_model.output[1].shape[1:].as_list()) #in1 = Input(shape=[None, None, 512])
    in2 = Input(shape=bottleneck_model.output[2].shape[1:].as_list()) #in2 = Input(shape=[None, None, 256])
