...
정우는 지금까지 USB를 두 번이나 뒤집어야 USB가 제대로 꽂히는 놀라운 경험을 너무 많이 했고, 
이런 상황이 얼마나 많이 일어날 수 있는지 알고 싶다.
...
for i in range(1,int(input())+1):
    ...
    p_r = 1-p # p의 반대, 뒤집어 꽂는것
    q_r = 1-q # q의 반대, 정상적으로 꽂히지 않음
    print(f"p : {p} , p_r : {p_r}")
    print(f"q : {q} , q_r : {q_r}")
    # 꽂힐 확률은 올바른 면(p)으로 usb를 꽂고 정상적으로 usb가 꽂혀야(q) 함 
    s1 = p*q#1번 뒤집었을 때 usb가 꽂힐 확률 
    s2 = p*p*q#2번 뒤집었을 때 usb가 꽂힐 확률 
    print(f"s1 : {s1} , s2 : {s2}")
    if s1 < s2:
        print('YES')
    else:
